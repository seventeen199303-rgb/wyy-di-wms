...
public class MovementOrderService {
    ...
    public TableDataInfo<MovementOrderVo> queryPageList(MovementOrderBo bo, PageQuery pageQuery) {
        ...
        fillLocationName(result.getRecords());
     ...
     * 填充库区/库位名称(源库区/源库位聚合)
     */
    private void fillLocationName(List<MovementOrderVo> list) {
        if (CollUtil.isEmpty(list)) {
            return;
        }
        Map<Long, Zone> zoneMap = zoneMapper.selectList().stream().collect(java.util.stream.Collectors.toMap(Zone::getId, z -> z, (a, b) -> a));
        Map<Long, Location> locMap = locationMapper.selectList().stream().collect(java.util.stream.Collectors.toMap(Location::getId, l -> l, (a, b) -> a));
        for (MovementOrderVo vo : list) {
            List<MovementOrderDetailVo> details = movementOrderDetailService.queryByMovementOrderId(vo.getId());
            if (CollUtil.isEmpty(details)) {
                continue;
            }
            // 源库位信息
            java.util.Set<String> srcZoneNames = new java.util.LinkedHashSet<>();
            java.util.Set<String> srcZoneCodes = new java.util.LinkedHashSet<>();
            java.util.Set<String> srcLocNames = new java.util.LinkedHashSet<>();
            java.util.Set<String> srcLocCodes = new java.util.LinkedHashSet<>();
            // 目标库位信息
            java.util.Set<String> tgtZoneNames = new java.util.LinkedHashSet<>();
            java.util.Set<String> tgtZoneCodes = new java.util.LinkedHashSet<>();
            java.util.Set<String> tgtLocNames = new java.util.LinkedHashSet<>();
            java.util.Set<String> tgtLocCodes = new java.util.LinkedHashSet<>();
            details.forEach(d -> {
                if (d.getSourceLocationId() != null && locMap.containsKey(d.getSourceLocationId())) {
                    Location loc = locMap.get(d.getSourceLocationId());
                    srcLocNames.add(loc.getLocationName());
                    srcLocCodes.add(loc.getLocationCode());
                    if (loc.getZoneId() != null && zoneMap.containsKey(loc.getZoneId())) {
                        Zone z = zoneMap.get(loc.getZoneId());
                        srcZoneNames.add(z.getZoneName());
                        srcZoneCodes.add(z.getZoneCode());
                    }
                }
                if (d.getTargetLocationId() != null && locMap.containsKey(d.getTargetLocationId())) {
                    Location loc = locMap.get(d.getTargetLocationId());
                    tgtLocNames.add(loc.getLocationName());
                    tgtLocCodes.add(loc.getLocationCode());
                    if (loc.getZoneId() != null && zoneMap.containsKey(loc.getZoneId())) {
                        Zone z = zoneMap.get(loc.getZoneId());
                        tgtZoneNames.add(z.getZoneName());
                        tgtZoneCodes.add(z.getZoneCode());
                    }
                }
            });
            vo.setZoneNames(String.join(",", srcZoneNames));
            vo.setZoneCodes(String.join(",", srcZoneCodes));
            vo.setLocationNames(String.join(",", srcLocNames));
            vo.setLocationCodes(String.join(",", srcLocCodes));
            vo.setTargetZoneNames(String.join(",", tgtZoneNames));
            vo.setTargetZoneCodes(String.join(",", tgtZoneCodes));
            vo.setTargetLocationNames(String.join(",", tgtLocNames));
            vo.setTargetLocationCodes(String.join(",", tgtLocCodes));
        }
    }
