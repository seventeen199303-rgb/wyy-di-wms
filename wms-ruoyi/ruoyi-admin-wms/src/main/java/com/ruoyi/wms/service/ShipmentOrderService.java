...
public class ShipmentOrderService {
     ...
     * 填充库区/库位名称(聚合去重)
     */
    private void fillLocationName(List<ShipmentOrderVo> list) {
        if (CollUtil.isEmpty(list)) {
            return;
        }
        Map<Long, Zone> zoneMap = zoneMapper.selectList().stream().collect(Collectors.toMap(Zone::getId, z -> z, (a, b) -> a));
        Map<Long, Location> locationMap = locationMapper.selectList().stream().collect(Collectors.toMap(Location::getId, l -> l, (a, b) -> a));
        for (ShipmentOrderVo vo : list) {
            ...
            Set<String> zoneNames = new java.util.LinkedHashSet<>();
            Set<String> zoneCodes = new java.util.LinkedHashSet<>();
            Set<String> locationNames = new java.util.LinkedHashSet<>();
