...
public class CheckOrderService {
     ...
     * 填充库区/库位名称(聚合去重)
     */
    private void fillLocationName(List<CheckOrderVo> list) {
        ...
        Map<Long, Location> locationMap = locationMapper.selectList().stream().collect(java.util.stream.Collectors.toMap(Location::getId, l -> l, (a, b) -> a));
        for (CheckOrderVo vo : list) {
            List<CheckOrderDetailVo> details = checkOrderDetailService.queryByCheckOrderId(vo.getId());
            if (cn.hutool.core.collection.CollUtil.isEmpty(details)) {
                continue;
            }
            java.util.Set<String> zoneNames = new java.util.LinkedHashSet<>();
            java.util.Set<String> zoneCodes = new java.util.LinkedHashSet<>();
            java.util.Set<String> locationNames = new java.util.LinkedHashSet<>();
            java.util.Set<String> locationCodes = new java.util.LinkedHashSet<>();
            details.forEach(d -> {
                if (d.getZoneId() != null && zoneMap.containsKey(d.getZoneId())) {
                    Zone z = zoneMap.get(d.getZoneId());
                    zoneNames.add(z.getZoneName());
                    zoneCodes.add(z.getZoneCode());
                }
                if (d.getLocationId() != null && locationMap.containsKey(d.getLocationId())) {
                    Location l = locationMap.get(d.getLocationId());
                    locationNames.add(l.getLocationName());
                    locationCodes.add(l.getLocationCode());
                }
            });
            vo.setZoneNames(String.join(",", zoneNames));
            vo.setZoneCodes(String.join(",", zoneCodes));
            vo.setLocationNames(String.join(",", locationNames));
            vo.setLocationCodes(String.join(",", locationCodes));
        }
    }
