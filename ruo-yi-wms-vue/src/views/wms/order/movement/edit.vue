      ...
      <el-card header="商品明细" class="mt10">
        <div class="receipt-order-content">
          ...
          <el-table :data="form.details" border empty-text="暂无商品明细">
            ...
            <el-table-column label="源库位" width="180">
              <template #default="scope">
                <el-select v-model="scope.row.sourceLocationId" placeholder="源库位" filterable clearable style="width: 100%">
                  <el-option v-for="loc in sourceLocationOptions" :key="loc.id" :label="loc.zoneName + ' - ' + loc.locationCode" :value="loc.id" />
                </el-select>
              </template>
            </el-table-column>
            <el-table-column label="目标库位" width="180">
              <template #default="scope">
                <el-select v-model="scope.row.targetLocationId" placeholder="目标库位" filterable clearable style="width: 100%">
                  <el-option v-for="loc in targetLocationOptions" :key="loc.id" :label="loc.zoneName + ' - ' + loc.locationCode" :value="loc.id" />
                </el-select>
              </template>
            </el-table-column>
...
const sourceLocationOptions = computed(() => {
  if (!form.value.sourceWarehouseId) return []
  const wmsStore = useWmsStore()
  const zoneMap = new Map(wmsStore.zoneList.map(z => [z.id, z.zoneName]))
  return wmsStore.locationList.filter(l => l.warehouseId === form.value.sourceWarehouseId).map(l => ({ ...l, zoneName: zoneMap.get(l.zoneId) || '' }))
})
// 目标库位选项（按目标仓库过滤）
const targetLocationOptions = computed(() => {
  if (!form.value.targetWarehouseId) return []
  const wmsStore = useWmsStore()
  const zoneMap = new Map(wmsStore.zoneList.map(z => [z.id, z.zoneName]))
  return wmsStore.locationList.filter(l => l.warehouseId === form.value.targetWarehouseId).map(l => ({ ...l, zoneName: zoneMap.get(l.zoneId) || '' }))
})
const initFormData = {
  id: undefined,
  orderNo: undefined,
