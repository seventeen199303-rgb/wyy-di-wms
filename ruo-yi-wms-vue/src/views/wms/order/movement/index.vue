  ...
  <div class="app-container">
    ...
    <el-card class="mt20">
      ...
      >
          ...
          <template #default="props">
            <div style="padding: 0 50px 20px 50px">
              ...
              <el-table :data="props.row.details" v-loading="detailLoading[props.$index]" empty-text="暂无商品明细">
                ...
                <el-table-column label="源库位" width="120">
                  <template #default="{ row }">
                    {{ useWmsStore().locationMap.get(row.sourceLocationId)?.locationCode || '-' }}
                  </template>
                </el-table-column>
                <el-table-column label="目标库位" width="120">
                  <template #default="{ row }">
                    {{ useWmsStore().locationMap.get(row.targetLocationId)?.locationCode || '-' }}
                  </template>
                </el-table-column>
        ...
        <el-table-column label="源仓库" align="left" min-width="100">
          <template #default="{ row }">
            <div>{{ useWmsStore().warehouseMap.get(row.sourceWarehouseId)?.warehouseName }}</div>
          </template>
        </el-table-column>
        <el-table-column label="源库区名称" prop="zoneNames" min-width="100">
          <template #default="{ row }">{{ row.zoneNames || '-' }}</template>
        </el-table-column>
        <el-table-column label="源库区编号" prop="zoneCodes" min-width="90">
          <template #default="{ row }">{{ row.zoneCodes || '-' }}</template>
        </el-table-column>
        <el-table-column label="源库位名称" prop="locationNames" min-width="100">
          <template #default="{ row }">{{ row.locationNames || '-' }}</template>
        </el-table-column>
        <el-table-column label="源库位编号" prop="locationCodes" min-width="90">
          <template #default="{ row }">{{ row.locationCodes || '-' }}</template>
        </el-table-column>
        <el-table-column label="目标仓库" align="left" min-width="100">
          <template #default="{ row }">
            <div>{{ useWmsStore().warehouseMap.get(row.targetWarehouseId)?.warehouseName }}</div>
          </template>
        </el-table-column>
        <el-table-column label="目标库区名称" prop="targetZoneNames" min-width="100">
          <template #default="{ row }">{{ row.targetZoneNames || '-' }}</template>
        </el-table-column>
        <el-table-column label="目标库区编号" prop="targetZoneCodes" min-width="90">
          <template #default="{ row }">{{ row.targetZoneCodes || '-' }}</template>
        </el-table-column>
        <el-table-column label="目标库位名称" prop="targetLocationNames" min-width="100">
          <template #default="{ row }">{{ row.targetLocationNames || '-' }}</template>
        </el-table-column>
        <el-table-column label="目标库位编号" prop="targetLocationCodes" min-width="90">
          <template #default="{ row }">{{ row.targetLocationCodes || '-' }}</template>
        </el-table-column>
