  ...
  <div class="app-container">
    ...
    <el-card class="mt20">
      ...
      >
            ...
            <div style="padding: 0 50px 20px 50px">
              ...
              <el-table :data="props.row.details" v-loading="detailLoading[props.$index]" empty-text="暂无商品明细">
                ...
                <el-table-column label="库区" width="120">
                  <template #default="{ row }">
                    {{ useWmsStore().zoneMap.get(row.zoneId)?.zoneName || '-' }}
                  </template>
                </el-table-column>
                <el-table-column label="库位" width="110">
                  <template #default="{ row }">
                    {{ useWmsStore().locationMap.get(row.locationId)?.locationCode || '-' }}
                  </template>
                </el-table-column>
        ...
        <el-table-column label="库区名称" prop="zoneNames" min-width="110">
          <template #default="{ row }">{{ row.zoneNames || '-' }}</template>
        </el-table-column>
        <el-table-column label="库区编号" prop="zoneCodes" min-width="100">
          <template #default="{ row }">{{ row.zoneCodes || '-' }}</template>
        </el-table-column>
        <el-table-column label="库位名称" prop="locationNames" min-width="110">
          <template #default="{ row }">{{ row.locationNames || '-' }}</template>
        </el-table-column>
        <el-table-column label="库位编号" prop="locationCodes" min-width="100">
