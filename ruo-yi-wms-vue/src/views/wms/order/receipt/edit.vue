      ...
      <el-card header="商品明细" class="mt10">
        <div class="receipt-order-content">
          ...
          <el-table :data="form.details" border empty-text="暂无商品明细">
            ...
            <el-table-column label="目标库位" width="200">
              <template #default="scope">
                <el-select v-model="scope.row.locationId" placeholder="请选择库位" filterable clearable style="width: 100%">
                  <el-option
                    v-for="loc in locationOptions"
                    :key="loc.id"
                    :label="loc.zoneName + ' - ' + loc.locationCode"
                    :value="loc.id"
                    @click="handleSelectLocation(scope.row, loc)"
                  />
                </el-select>
              </template>
            </el-table-column>
...
// 目标库位选项（按仓库过滤，显示库区名-库位编码）
const locationOptions = computed(() => {
  if (!form.value.warehouseId) return []
  const zoneMap = new Map(allZones.value.map(z => [z.id, z.zoneName]))
  return allLocations.value
    .filter(l => l.warehouseId === form.value.warehouseId)
    .map(l => ({ ...l, zoneName: zoneMap.get(l.zoneId) || '' }))
})

function handleSelectLocation(row, loc) {
  row.locationId = loc.id
  row.zoneId = loc.zoneId
}
const initFormData = {
  id: undefined,
  orderNo: undefined,
...
onMounted(() => {
  // 加载库位/库区列表（用于目标库位选择）
  listLocationNoPage({}).then(res => { allLocations.value = res.data || [] })
  listZoneNoPage({}).then(res => { allZones.value = res.data || [] })
