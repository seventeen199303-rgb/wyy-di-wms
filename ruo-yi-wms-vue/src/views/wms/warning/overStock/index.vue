<template>
  <div class="app-container">
    ...
    <el-card class="mt20">
      <el-alert title="超储预警：当前库存超过最大库存上限的物料" type="error" :closable="false" class="mb10" show-icon />
      <el-table v-loading="loading" :data="list" border empty-text="暂无预警">
        ...
        <el-table-column label="超储数量" prop="difference" width="100" align="right">
...
<script setup name="OverStock">
import { listOverStockWarning } from '@/api/wms/warning'
...
function getList() {
  loading.value = true
  listOverStockWarning(queryParams.value).then(res => {
    list.value = res.rows
